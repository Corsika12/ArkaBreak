...
struct GameView: View {
    ...
    @StateObject private var levelsVM: LevelsVM
    ...
    init() {
        ...
        let bonusVM = BonusVM()
        let levelsVM = LevelsVM()
        let gameEngineVM = GameEngineVM(paddleVM: paddleVM, bonusVM: bonusVM, levelManager: levelsVM)

        _paddleVM = StateObject(wrappedValue: paddleVM)
        _bonusVM = StateObject(wrappedValue: bonusVM)
        _levelsVM = StateObject(wrappedValue: levelsVM)
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            .task {
                await MainActor.run {
                    if !didStartGame, geo.size.width > 0, geo.size.height > 0 {
        ...
        Group {
            if let countdown = countdownManager.text {
                ...
                Text(countdown)
                    ...
                    .scaleEffect(countdown == "GO!" ? 1.6 : 1.2)
        ...
        Group {
            ...
            ForEach(bonusVM.bonuses) { bonus in
                ZStack {
                    ...
                    Circle().fill(bonus.type.color)
                    Circle().stroke(bonus.type.color.opacity(1), lineWidth: 2.5)
